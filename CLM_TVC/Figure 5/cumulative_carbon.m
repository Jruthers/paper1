...
ix2=find(ix > 200); % cuts off rows pertaining to start of snow season
ix3=min(ix(ix2)); % this finds the rows pertaining to end of season (i.e. >200) and then gets the minimum row (ie earliest snow off)
p3 = plot([1:ix3], (SRi_J_2046sum(1:ix3,1)), 'k-');
hold on
ix=find(diff(SRi_J_2096sum)==0); % find rows where the difference between consectiuve vals =0
ix2=find(ix > 200); % cuts off rows pertaining to start of snow season
ix3=min(ix(ix2)); % this finds the rows pertaining to end of season (i.e. >200) and then gets the minimum row (ie earliest snow off)
p4 = plot([1:ix3], (SRi_J_2096sum(1:ix3,1)), 'r-');
set(gca, 'xtick' ,[0:20:304]);
hold on
xlim([0 300])
...
xlabel('Days since 1^{st} Sep')
%% plot options
annotation('textbox', [0.436448784376245 0.869630549190156 0.0329015544041451 0.0536062378167641], 'String', '(a)', 'EdgeColor', 'none', 'FontSize', 14, 'FontWeight', 'normal')
annotation('textbox', [0.875316859306496 0.869630549190156 0.0334196891191709 0.0536062378167641], 'String', '(b)', 'EdgeColor', 'none', 'FontSize', 14, 'FontWeight', 'normal')
annotation('textbox', [0.436448784376245 0.396297215856823 0.0329015544041451 0.053606237816764], 'String', '(c)', 'EdgeColor', 'none', 'FontSize', 14, 'FontWeight', 'normal')
annotation('textbox', [0.875316859306496 0.396297215856823 0.0334196891191711 0.0536062378167641], 'String', '(d)', 'EdgeColor', 'none', 'FontSize', 14, 'FontWeight', 'normal')
%% save plot
