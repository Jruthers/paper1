...
function [h, u] = raincloud_plot(X,dropsizes, varargin)
...
yl = get(gca, 'YLim');
yl = [0 0.3]
set(gca, 'YLim', [-yl(2)*2.3 yl(2)]); % lwr_bnd multiplies -yl(2)
...
% raindrops
if box_dodge
    drops_pos = (jit * 3) - yl(2) * dot_dodge_amount;
else
    drops_pos = jit*3 - yl(2) / 2; % change the number multipling jit to spread out the dots
end

h{2} = scatter(X, drops_pos);
dropsizes(dropsizes==0)=0.1;
h{2}.SizeData = dropsizes;
