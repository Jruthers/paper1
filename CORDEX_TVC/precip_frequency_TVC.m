...
for j = 1:length(FilesList)
    ...
    % Convert the date columns (year, month, day) into a datenum format
    datestamp = datenum(num2str(table2array([Output(:,1),Output(:,2),Output(:,3)])));
    
    % Identify unique combinations of year and month in the data
    ...
    for i = 1:max(RowsofEachUniqueCombo)
        % Find the rows corresponding to the current year-month combination
        rowsneeded = find(RowsofEachUniqueCombo == i);
        
        % Calculate and store monthly mean and total values for precipitation and temperature
...
% Convert monthly precipitation from average rate to total volume
HPCmonthlyprecip = HPCmonthlyprecip * 60 * 60 * 24;

% Combine time and calculated values into final arrays
